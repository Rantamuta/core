...
Exception (documentation-only tasks):

- If the approved plan contains **only documentation changes** and no code or test changes, Phase 4A may be skipped.
- In that case, explicitly note “Doc-only exception: no failing test required” and proceed to Phase 5.
